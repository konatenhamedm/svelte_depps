<script lang="ts">
  ...
  async function SaveFunction() {
    isLoad = true;
    try {
      const res = await fetch(BASE_URL_API + "/racineSequence/create", {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        isLoad = false;
        open = false;
        notificationMessage = "racine créé avec succès!";
...
<Modal
  bind:open
  title={Object.keys(data).length ? "Ajouter une racine " : "Ajouter une racine"}
  size={sizeModal}
