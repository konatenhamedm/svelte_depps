	...
	export let data: Record<string, string> = {};
	 ...
	 async function SaveFunction() {
        ...
        try {
            ...
            const res = await fetch(BASE_URL_API +`/typePersonne/delete/`+data?.id, {
                method: 'DELETE',
               /*  headers: {
...
<Button type="submit" color="red" style="background-color: red;" on:click={SaveFunction} class="mr-2">Supprimer</Button>
            {/if}
       <!--  </div> -->
<Button color="dark" style="background-color: black;" on:click={() => (open = false)}>Annuler</Button>	</div>
</Modal>