      ...
      <div class="bg-white rounded-lg shadow p-4 border border-gray-100 flex flex-col">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center">
            <button on:click={toggleAmount} class="text-gray-500 hover:text-gray-700 mr-2">
              <svg
            ...
            {:else}
              {#each filteredProfessionnels.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage) as item}
                <TableBodyRow class="text-base border border-gray-300">
                  <TableBodyCell class="p-4 border border-gray-300">{item.personne?.nom ?? 'N/A'}</TableBodyCell>
                  <TableBodyCell class="p-4 border border-gray-300">{item.personne?.prenoms ?? 'N/A'}</TableBodyCell>
                  <TableBodyCell class="p-4 border border-gray-300">{item.personne?.number ?? 'N/A'}</TableBodyCell>
                  <TableBodyCell class="p-4 border border-gray-300">{item.personne?.email ?? 'N/A'}</TableBodyCell>
                  <TableBodyCell class="p-4 border border-gray-300">{item.personne?.profession?.libelle ?? 'N/A'}</TableBodyCell>
                </TableBodyRow>
              {/each}
            {/if}
          </TableBody>
      ...
      <div class="w-full grid grid-cols-4">
        <div class="col-span-3 p-2">
          <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
            ...
            sur un total de
            <span class="font-semibold text-gray-900 dark:text-white">{totalPages * itemsPerPage}</span>
          </span>
        </div>
