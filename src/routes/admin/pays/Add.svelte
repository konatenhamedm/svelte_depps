<script lang="ts">
  ...
  async function SaveFunction() {
    isLoad = true;
    try {
      const res = await fetch(BASE_URL_API + "/pays/create", {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        isLoad = false;
        open = false;
        notificationMessage = "situation professionnelle créé avec succès!";
...
<Modal
  bind:open
  title={Object.keys(data).length ? "Ajouter un pays " : "Ajouter un pays"}
  size={sizeModal}
