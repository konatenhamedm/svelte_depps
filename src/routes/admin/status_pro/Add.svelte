<script lang="ts">
  ...
  async function SaveFunction() {
    isLoad = true;
    try {
      const res = await fetch(BASE_URL_API + "/statusPro/create", {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        isLoad = false;
        open = false;
        notificationMessage = "Statut professionnel  créé avec succès!";
...
<Modal
  bind:open
  title={Object.keys(data).length ? "Ajouter un statut professionnel  " : "Ajouter un statut professionnel "}
  size={sizeModal}
