<script lang="ts">
  ...
  async function SaveFunction() {
    isLoad = true;
    try {
      const res = await fetch(BASE_URL_API + "/typeProfession/create", {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        isLoad = false;
        open = false;
        notificationMessage = "type profession créé avec succès!";
...
<Modal
  bind:open
  title={Object.keys(data).length ? "Ajouter une type profession " : "Ajouter une type profession"}
  size={sizeModal}
