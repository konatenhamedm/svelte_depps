<script lang="ts">
  ...
  async function SaveFunction() {
    isLoad = true;
    try {
      const res = await fetch(BASE_URL_API + "/typeDiplome/create", {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        isLoad = false;
        open = false;
        notificationMessage = "type diplome créé avec succès!";
...
<Modal
  bind:open
  title={Object.keys(data).length ? "Ajouter un type diplome " : "Ajouter un type diplome"}
  size={sizeModal}
