  ...
  function updateFilteredData() {
    if (selectedProfession === '') {
      ...
      filteredProfessionnels = professionnels.filter(p =>
              p.profession?.id === selectedProfession ||
              p.profession?.libelle === selectedProfession
      );
      ...
      filteredEtablissements = etablissements.filter(e =>
              e.profession?.id === selectedProfession ||
              e.profession?.libelle === selectedProfession
      );
    }
  }
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            {#each filteredProfessionnels.slice((currentPage-1)*itemsPerPage, currentPage*itemsPerPage) as item}
              <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3 whitespace-nowrap text-sm">{item.profession?.libelle ?? 'N/A'}</td>
              </tr>
            {:else}
              <tr>
