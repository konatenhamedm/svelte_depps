<script lang="ts">
  ...
  const tabs = [
    ...
    { key: "oep_dossier_conforme", label: "Validé" },
    { key: "oep_dossier_non_conforme", label: "Rejeté" },
  ];
...
<Entete
  ...
  parent="Ouverture d'exploitation"
      ...
      <div class="box">
        <div class="box-header with-border flex justify-between items-center">
          <h4 class="box-title text-xl font-medium">
            Liste des dossiers d'etablissements
          </h4>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
            ...
            <Table class="border border-gray-300">
              ...
              >
                {#if user.type == "ADMINISTRATEUR"}
                  {#each ["email", "Niveau d'Intervention", "Téléphone/Adresse", "Entité Juridique", "imputation"] as title}
                    {#if title}
                      <TableHeadCell
                ...
                {:else}
                  {#if activeTab === "valide"}
                    {#if user.type === "SOUS-DIRECTEUR"}
                      {#each ["email", "Niveau d'Intervention", "Téléphone/Adresse", "Entité Juridique", "Imputation", "Action"] as title}
                        {#if title}
                          <TableHeadCell
                            class="ps-4 font-normal border border-gray-300"
                          >
                    ...
                    {:else}
                      ...
                      {/each}
                    {/if}
                  {:else if activeTab == "oep_dossier_conforme"}
                    {#each ["email", "Niveau d'Intervention", "Téléphone/Adresse", "Entité Juridique", "Imputation", "code", "Action"] as title}
                      {#if title}
                        <TableHeadCell
                          class="ps-4 font-normal border border-gray-300"
                        >
              ...
              <TableBody>
                ...
                {:else if activeTab == "oep_dossier_conforme"}
                    ...
                    <TableBodyRow class="text-base border border-gray-300">
                      ...
                      >
                      <TableBodyCell class="p-4 border border-gray-300"
                        >{item.personne.niveauIntervention
                          .libelle}</TableBodyCell
                      >
                      <TableBodyCell class="p-4 border border-gray-300"
                        >{item.personne.telephone
                          ? item.personne.telephone
                          : item.personne.adresse}</TableBodyCell
                      >
