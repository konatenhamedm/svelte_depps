<script lang="ts">
  ...
  let recipients: any = [];
   ...
   async  function handleSubmit(event: any) {
      ...
      await apiFetch(true, "/alerte/update/" + forum?.id, "POST", updatedForum).then((res) => {
        if (res) {
          loading = false;
          closePopup();
        }
        ...
        <div class="bg-white p-6 rounded-lg shadow-lg w-[43em] forum-con">
            ...
            <form on:submit={handleSubmit}>
                <div class="mb-4">
                    ...
                    <input type="text" name="objet" value={forum.objet} class="w-full px-3 py-2 border rounded" required />
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 mb-2">Message</label>
                    <textarea name="message" class="w-full px-3 py-2 border rounded" required>{forum.message}</textarea>
                </div>
                <div class="mb-4">
                  ...
                  >
                    {#each recipients as recipient}
                      <option value={recipient.id} selected={forum.destinateur.id === recipient.id }>{recipient.libelle}</option>
                    {/each}
                  </select>
                </div>
